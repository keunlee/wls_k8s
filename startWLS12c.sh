...
if [[ "${SERVER_TYPE}" == "AdminServer" ]]; then
   ...
   nohup java -Djava.security.egd=file:/dev/./urandom weblogic.WLST $SCRIPT_PATH/startWLS12c_admin.jy &
   else if [[ "${SERVER_TYPE}" == "ManagedServer" ]]; then
   echo "Calling script to start ManagedServer"
   nohup java -Djava.security.egd=file:/dev/./urandom weblogic.WLST $SCRIPT_PATH/startWLS12c_managed.jy &
        else
